...
import { Target, Loader2, Award, User, Hash, Clock } from "lucide-react";
...
const DriverTargetTripsPage = () => {
    ...
    const fetchDriverCredentials = async () => {
      ...
      if (driverIdFromEmail) {
        try {
          ...
          if (!credError && credentials?.driver_id) { setDriverId(credentials.driver_id); return; }
        } catch (error) { console.error("Error with RPC get_driver_credentials:", error); }
      }

      try {
        const { data: credentials, error } = await supabase.from("driver_credentials").select("driver_id").eq("user_id", actualUserId).eq("status", "enabled").maybeSingle();
        if (error) throw error;
        if (credentials) setDriverId(credentials.driver_id);
      } catch (error: any) { console.error("Error fetching driver credentials:", error); }
    };
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const { data: tripsData, error: tripsError } = await supabase.from("target_trips").select("*").eq("driver_id", driverId).order("created_at", { ascending: false });
        if (tripsError) throw tripsError;
        setTargetTrips(tripsData || []);
        if (tripsData && tripsData.length > 0 && tripsData[0].driver_name) setDriverName(tripsData[0].driver_name);

        const { data: incomeData, error: incomeError } = await supabase.from("driver_income").select("shift, working_days, total_trips, driver_name").eq("driver_id", driverId).order("created_at", { ascending: false }).limit(1).maybeSingle();
        if (incomeError) throw incomeError;
        setDriverIncome(incomeData);
        if (incomeData?.driver_name && !tripsData?.[0]?.driver_name) setDriverName(incomeData.driver_name);
      } catch (error: any) {
  ...
  const baseAvgTripsPerDay = useMemo(() => {
    if (latestTrip && latestTrip.target_trips > 0) return latestTrip.target_trips;
  ...
  const tierData: TierData[] = useMemo(() => {
    return DEFAULT_TIERS.map((tierName, i) => {
      ...
      return { tier: tierName, avgTripsPerDay: avgTrips, totalTripsMonth: Math.round(avgTrips * daysInMonth), incentive };
    });
      ...
      {loading ? (
        ...
        <Card className="bg-white/5 backdrop-blur-lg border-white/10">
          <CardContent className="p-10 text-center">
            ...
            <Button className="mt-6 bg-emerald-600 hover:bg-emerald-700" onClick={() => navigate("/login")}>Login with Driver Account</Button>
          </CardContent>
        </Card>
                ...
                <div className="space-y-3">
                  <div>
                    <div className="flex items-center gap-2 text-white/50 text-xs mb-1">
                      <Hash className="h-3 w-3" /><span>ID No</span>
                    </div>
                    <p className="text-2xl font-bold text-white">{driverId}</p>
                  </div>
                  <div>
                    <p className="text-white/50 text-xs mb-1">Name:</p>
                    <p className="text-base font-medium text-white/90">{driverName || "Driver"}</p>
                  </div>
                </div>
                <div className="text-right">
                  <div className="flex items-center gap-2 text-white/50 text-xs mb-2 justify-end">
                    <Clock className="h-3 w-3" /><span>Shift:</span>
                  </div>
                  <Badge className="bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 text-xl px-4 py-2 font-bold border border-emerald-500/30">{shiftLabel}</Badge>
                </div>
              </div>
            </CardContent>
          </Card>

          {/* Target & Incentive Tiers Table */}
          <Card className="bg-white/5 backdrop-blur-lg border-white/10 overflow-hidden">
                ...
                <table className="w-full">
                    ...
                    <tr className="border-b border-white/10">
                      ...
                      <th className="text-center px-4 py-3 text-sm font-medium text-white/70">
                        <div className="flex flex-col items-center leading-tight">
                          <span>Avg</span><span className="text-[10px] font-normal text-white/40">Trip/Day</span>
                        </div>
                      </th>
                      <th className="text-center px-4 py-3 text-sm font-medium text-white/70">
                        <div className="flex flex-col items-center leading-tight">
                          <span>Total Trips/</span><span className="text-[10px] font-normal text-white/40">Month</span>
                        </div>
                      </th>
                  ...
                  <tbody>
                    {tierData.map((tier, index) => (
                      <tr key={tier.tier} className={`border-b border-white/5 transition-colors ${index % 2 === 0 ? "bg-white/[0.02]" : ""} hover:bg-white/[0.05]`}>
                        <td className="px-5 py-3"><span className="font-medium text-white/90">{tier.tier}</span></td>
                        <td className="px-4 py-3 text-center"><span className="text-white/70 font-medium">{tier.avgTripsPerDay.toFixed(2)}</span></td>
                        <td className="px-4 py-3 text-center"><span className="font-semibold text-white">{tier.totalTripsMonth}</span></td>
                        <td className="px-5 py-3 text-center">
                          <span className="inline-flex items-center justify-center min-w-[60px] px-3 py-1.5 rounded-full bg-gradient-to-r from-emerald-500/20 to-teal-500/20 text-emerald-400 font-bold text-sm border border-emerald-500/20">{tier.incentive}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
