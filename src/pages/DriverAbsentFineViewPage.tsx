...
const DriverAbsentFineViewPage = () => {
  ...
  useEffect(() => {
    const fetchDriverId = async () => {
      ...
      if (driverIdFromEmail) { setDriverId(driverIdFromEmail); return; }
  ...
  const fetchFines = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const { data, error } = await supabase.from("driver_absent_fines").select("*").eq("driver_id", driverId).order("timestamp", { ascending: false });
      if (error) { console.error("Error fetching fines:", error); return; }
      setFines(data || []);
    } catch (err) { console.error("Error:", err); }
    finally { setIsLoading(false); }
  }, [driverId]);

  useEffect(() => { if (driverId) fetchFines(); }, [driverId, fetchFines]);

  const formatDate = (dateStr: string) => {
    try { return new Date(dateStr).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: '2-digit' }); }
  ...
  return (
    <PageLayout
      title="My Absent Fines"
      icon={<Ban className="h-8 w-8 text-orange-600" />}
      backPath="/driver-portal"
      backLabel="Back to Portal"
      maxWidth="4xl"
      gradient="from-background via-orange-50/50 to-red-100/50"
    >
      {driverId && (
              ...
              <TableBody>
                {fines.map((fine) => (
                  <TableRow key={fine.id}>
                    ...
                    <TableCell className="text-right font-semibold text-destructive">AED {Number(fine.total_amount).toFixed(2)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
